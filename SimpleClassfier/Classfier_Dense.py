...
if __name__ == '__main__':
    batch_size = 16
    model_path = 'classfier_model/classfier_Norm_B_and_A_withEval_Densenet.pt'
    ...
    if os.path.exists(model_path):
        ...
        model = DenseNet()
    model.cuda()
    optimizer = optim.Adam(filter(lambda p: p.requires_grad, model.parameters()), lr=1e-3,weight_decay=0.00005)
    loss_func = ClassLoss()
    epochs = 200
    best_epochs = 0
    max_score = 0.43
    for i in range(epochs):
        ...
        for idx, batch_data in enumerate(train_dataloader):
            ...
            label_one_hot = batch_data['label_hot'].long().cuda()
            model.zero_grad()
            loss = model.get_loss(images,label_num)
            count += 1
            sum_loss += float(loss)
            loss.backward()
            optimizer.step()
        ...
        if score > max_score:
            ...
            print("score declined:" + str(score - max_score))
